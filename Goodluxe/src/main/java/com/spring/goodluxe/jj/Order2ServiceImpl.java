...
public class Order2ServiceImpl implements Order2Service{
	...
	@Override
	public int insertboardandorder(ProductBoardVO pbvo)throws Exception{
		int res=0;
		
		try {
			Order2Mapper orderMapper = sqlSession.getMapper(Order2Mapper.class);
			res = orderMapper.insertboardandorder(pbvo);
		}catch(Exception e) {
			System.out.println("board insert fail ="+e.getMessage());
		}
		return res;
	}
}
