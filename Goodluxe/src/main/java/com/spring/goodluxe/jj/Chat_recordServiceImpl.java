...
public class Chat_recordServiceImpl implements Chat_recordService{
	...
	@Override
	public int insertuserchatRecord(Chat_recordVO chatrecordvo)throws Exception{
		int res = 0;
		try {
			Chat_recordMapper chat_recordMapper = sqlSession.getMapper(Chat_recordMapper.class);
			res = chat_recordMapper.insertuserchatRecord(chatrecordvo);
		}catch(Exception e) {
			System.out.println("chatrecord user add fail+"+e.getMessage());
		}
		return res;
	}
