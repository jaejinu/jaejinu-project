...
public void afterConnectionEstablished(WebSocketSession session) throws Exception {
	...
	ArrayList<String> roomchk = new ArrayList<String>();
	...
	for(int i=0; i<sessionList.size(); i++) { 
		...
		if(userRoom.getChat_room().equals(roomList.get(sessionList.get(i))) && !roomchk.contains(roomName)) { //방이 같다면 
			//sessionList.get(i).sendMessage(new TextMessage(JsonDataOpen(userId))); //메시지를 보내줘 
				roomchk.add(roomName);
			for (WebSocketSession webSocketSession : sessionList) { // 세션를 향상된 포문으로 돌려 web session 갯수만큼
				//자신한테만 보내도록 함
				if(session.getId().equals(webSocketSession.getId())) { //세견 아디가 웹소켓 아디랑 같으면 
			//대화 내용 뿌리기 
			for(int j =0; j< recordlist.size(); j++) { //대화 내용을 뿌려 
	...
	if(message.getPayload().contains("!%/&")) {
		...
		System.out.println("보낸메시지:"+msgArr[0]+", 방의이름:"+msgArr[1]);
			...
			if(member_id.equals("admin")) {
				...
				chat_recordService.insertuserchatRecord(chat_recordvo);
			}else {
...
public String JsonData(String id, Object msg) {
	...
	jsonObject = Json.createObjectBuilder().add("message",
			...
			"<div id="+id+" width='100%'   "
					...
					+ "</span> "
					+ "<textarea class='speech-bubble' background='#ffffff' cols='30' rows='3' wrap='virtual' or 'physical' or 'off' readonly>"
	+msg+
	"</textarea>"
