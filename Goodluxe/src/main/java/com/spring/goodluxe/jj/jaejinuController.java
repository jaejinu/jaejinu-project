...
public class jaejinuController {
	...
	public String chat(Model model, HttpSession session) throws Exception {
		...
		System.out.println("selectchatrecordlist");
	...
	// give me the chat_list! 
		@RequestMapping(value="/ajaxchat_room.do", method=RequestMethod.GET,produces="application/json;charset=UTF-8")
		@ResponseBody
		public ArrayList<Chat_recordVO> ajaxchat_room(Model model)throws Exception {
			ArrayList<Chat_recordVO> chatrecordcountlist = new ArrayList<Chat_recordVO>();
			
			try {
				chatrecordcountlist = chat_recordService.selectListChatRecordcountdo();
				model.addAttribute("chatrecordcountlist", chatrecordcountlist);
				//System.out.println("chatrecordcountlist"+chatrecordcountlist);
			}catch(Exception e) {
				System.out.println("ajax chat_room list +"+e.getMessage());
			}
	...
	public String usesrCheckMember(Member2VO vo, HttpSession session, HttpServletResponse response) {
		...
		try {
			...
			if (res == 1) {
				...
				writer.write("<script>alert('유저체크 및 이름 가져오기 성공!!'); location.href='./client_chat.do?member_id="
						+ member_id + "';</script>");
			} else {
				writer.print("<script>alert('유저체크 실패!!');location.href='./loginForm.do';</script>");
			}
	...
	public String auction_detail(@RequestParam("file_1") List<MultipartFile> fileList_1,
		...
		for (MultipartFile mf_1 : fileList_1) {
			...
			try {
				...
				case 0:
					auctionvo.setAUCTION_PHOTO1_STORED(storedFileName_1);
					break;
				case 1:
					auctionvo.setAUCTION_PHOTO2_STORED(storedFileName_1);
					break;
				case 2:
					auctionvo.setAUCTION_PHOTO3_STORED(storedFileName_1);
					break;
				case 3:
					auctionvo.setAUCTION_PHOTO4_STORED(storedFileName_1);
					break;
				default:
					break;
		...
		try {
			...
			pbvo.setPb_number(AUCTION_POST_NUMBER+"");
			pbvo.setPb_division("purchase");
			pbvo.setPb_md_name(auctionvo.getAUCTION_MD_NAME());
			pbvo.setPb_sale_status("거래진행중");
			pbvo.setPb_post_status("게시안함");
			pbvo.setPb_detail("게시안함");
			pbvo.setPb_main_img_original(auctionvo.getAUCTION_PHOTO1_STORED());
			pbvo.setPb_main_img_stored(auctionvo.getAUCTION_PHOTO1_STORED());
			pbvo.setPb_detail_img1_original(auctionvo.getAUCTION_PHOTO2_STORED());
			pbvo.setPb_detail_img1_stored(auctionvo.getAUCTION_PHOTO2_STORED());
			pbvo.setPb_detail_img2_original(auctionvo.getAUCTION_PHOTO3_STORED());
			pbvo.setPb_detail_img2_stored(auctionvo.getAUCTION_PHOTO3_STORED());
			pbvo.setPb_detail_img3_original(auctionvo.getAUCTION_PHOTO4_STORED());
			pbvo.setPb_detail_img3_stored(auctionvo.getAUCTION_PHOTO4_STORED());
			pbvo.setPb_view_count(0);
			pbvo.setPb_like(0);			
			pbvo.setEntity_number(entitynum);
			
			
			orderService.insertboardandorder(pbvo);
	...
	public String historyinsert(RedirectAttributes redirectAttributes, Model model_t, HttpServletRequest request,  SessionStatus status)throws Exception {
		...
		int bettingchk = historyService.selecthistorycount(auction_post_number);
				if(bettingchk > 0) {
		int bettingprice = historyService.auctionhistoryselectone(auction_post_number).getAUHIS_BETTING_PRICE();
		if(bettingprice>=auhis_betting_price) {
		...
		historyvo.setAUHIS_BETTING_PRICE(auhis_betting_price);
	 ...
	 * // 입찰하기
	 * 
	 * @RequestMapping(value = "/history.do", method = RequestMethod.GET, produces =
	 * "application/json;charset=UTF-8")
	 * 
	 * @ResponseBody public String historyinsert(Model model_t, HttpServletRequest
	 * request) { int auction_post_number =
	 * Integer.parseInt(request.getParameter("auction_post_number"));
	 * System.out.println(auction_post_number); String member_id =
	 * request.getParameter("member_id"); System.out.println(member_id);
	 * System.out.println(request.getParameter("AUHIS_BETTING_PRICE")); int
	 * auhis_betting_price =
	 * Integer.parseInt(request.getParameter("AUHIS_BETTING_PRICE"));
	 * System.out.println(auhis_betting_price);
	 * 
	 * Auction_HistoryVO historyvo = new Auction_HistoryVO();
	 * System.out.println(auction_post_number);
	 ...
	 * 
	 * int count = 0; int AUCTION_POST_NUMBER = historyvo.getAUCTION_POST_NUMBER();
	 * System.out.println("AUCTION_POST_NUMBER=" + auction_post_number); try {
	 * System.out.println("history"); historyService.insertHistory(historyvo);
	 * System.out.println("history"); AuctionVO vo_2 =
	 * auctionService.selectAuction_PostNumber(auction_post_number);
	 * System.out.println(vo_2.getENTITY_NUMBER());
	 * System.out.println(vo_2.getAUCTION_DETAIL());
	 * System.out.println(vo_2.getAUCTION_MD_NAME()); System.out.println("history");
	 * String entitynumber = vo_2.getENTITY_NUMBER();
	 * 
	 * Purchase2VO purchasevo = purchaseService.selectPurchase(entitynumber);
	 * System.out.println("history"); model_t.addAttribute("purchasevo",
	 * purchasevo);
	 * 
	 * model_t.addAttribute("auctionvo", vo_2); count =
	 * historyService.selecthistorycount(auction_post_number);
	 * model_t.addAttribute("auctionnumber", count);
	 * 
	 * } catch (Exception e) { System.out.println("입찰 실패 +" + e.getMessage()); }
	 * return "auction_detail";
	 * 
	 * }
	 */
...
//		model_t.addAttribute("auctionvo", vo);
	...
	public int insertJSON(HttpServletRequest request)throws Exception {
		...
		try {
			...
			ordervo.setOrder_order_date(order_order_date);
			ordervo.setOrder_ship_date(order_ship_date);
			ordervo.setOrder_cancel_date(order_cancel_date);
	...
	public @ResponseBody String keywordSearch(@RequestParam("entity_number") String entity_number,
		...
		List keywordList = purchaseService.selectKeywordSearch(entity_number);
		 ...
		 * 
		 * 
		 * retVal.put("keywordList", keywordList); //bookList란 키로 bookList의 값을 넣어줍니다.
		 * (웹에서 bookList키로 추출
		 * 
		 * retVal.put("code", "OK");
		 * 
	...
	public ModelAndView searchPurchase(@RequestParam("searchWord") String searchWord, HttpServletRequest request,
